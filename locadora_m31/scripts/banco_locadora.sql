...
INSERT INTO usuario(usuario,senha) VALUES ('Matheus','12345');
...
SELECT c.carro, c.valor, t.tipo, m.montadora FROM carro c INNER JOIN tipo t ON t.cod_tipo = c.tipo_carro INNER JOIN montadora m ON m.cod_montadora = c.montadora_carro;
...
ALTER TABLE cliente ADD cpf varchar(15) not null;
...
SELECT c.carro, b.bairro FROM carro c
INNER JOIN tipo t ON t.cod_tipo=c.tipo_carro
INNER JOIN montadora m ON m.cod_montadora=c.montadora_carro
INNER JOIN carros_locacao o ON o.carro_locado = c.cod_carro
INNER JOIN locacao a ON a.cod_locacao = o.locacao
INNER JOIN cliente i ON i.cod_cliente = a.cliente_locacao
INNER JOIN bairro b ON b.cod_bairro=b.cod_bairro;
